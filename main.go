...
func getRtmToken(c *gin.Context) {
	...
	if err != nil {
	 ...
	 // Convert expireTimestamp from uint32 to string
	 expireTimestampStr := strconv.FormatUint(uint64(expireTimestamp), 10)

	rtmToken, tokenErr := rtmtokenbuilder2.BuildToken(appID, appCertificate, uidStr, 1, expireTimestampStr)
...
func getBothTokens(c *gin.Context) {
    ...
    // generate rtmToken
    rtmToken, rtmTokenErr := rtmtokenbuilder2.BuildToken(appID, appCertificate, uidStr, 1, expireTimestampStr)
